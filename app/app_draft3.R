...
server <- function(input, output, session) {
  ...
  #   leaflet() %>%
  #     addTiles() %>%
  #     addPolygons(data = states_tomap(),
  #                 fillColor = "gray",
  #                 color = "gray",
  #                 weight = 1,
  #                 fillOpacity = 0.7)
  # })
  # # update leaflet map when states are selected
  ...
  #                 fillOpacity = 0.7)
  # })
  # # remove existing layers when states are deselected
  # observeEvent(input$states, {
  #   proxy %>%
  #     removeShape(states_tomap())
  # })
  # 
  
  # observe({
  #   states_map <- states_tomap()
  #   
  #   leafletProxy("leaflet", data = states_map) %>%
  #     # In this case either lines 107 or 108 will work
  #     # clearShapes() %>%
  #     clearGroup(group = "states_map") %>%
  #     addPolygons(data =color = 'gray')
  #     # addPolygons(popup = ~paste0("<b>", NAME, "</b>"), group = "states_map", layerId = ~STATE, fill = FALSE, color = "green")# %>%
  #     #setView(lng = states_map$long, lat = states_map$lat, zoom = 9)
  # })
